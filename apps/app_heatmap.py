...
hm_inputs_charts_regex = dbc.FormGroup(
    [
        dbc.Label('charts regex', id='hm-label-charts-regex', html_for='hm-input-charts-regex', style={'margin': '4px', 'padding': '0px'}),
        dbc.Input(id='hm-input-charts-regex', value='system.*|apps.*|users.*|groups.*', type='text', placeholder='system.*'),
...
hm_inputs_before = dbc.FormGroup(
    [
...
hm_inputs_opts = dbc.FormGroup(
    [
        dbc.Label('options', id='hm-label-opts', html_for='hm-input-opts', style={'margin': '4px', 'padding': '0px'}),
        dbc.Input(id='hm-input-opts', value=DEFAULT_HM_OPTS, type='text', placeholder=DEFAULT_HM_OPTS),
        dbc.Tooltip('list of key values to pass to underlying code.', target='hm-label-opts')
    ]
)
hm_inputs = dbc.Row(
    [
        dbc.Col(hm_inputs_host, width=3),
        dbc.Col(hm_inputs_charts_regex, width=2),
        dbc.Col(hm_inputs_after, width=2),
        dbc.Col(hm_inputs_before, width=2),
        dbc.Col(hm_inputs_opts, width=2),
...
def hm_run(n_clicks, tab, host, charts_regex, after, before, opts, freq='10s', w='1200'):
    figs = []
